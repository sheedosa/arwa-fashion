...
export function Dialog({ title, onClose, width, children, className, panelStyle }: { title: string; onClose: () => void; width?: number; children: ReactNode; className?: string; panelStyle?: CSSProperties }) {
  const { t } = useI18n()
  const style: CSSProperties = { ...(width ? { width: `min(${width}px, 100%)` } : {}), ...panelStyle }
  const titleId = useId()
  ...
  return (
    ...
    >
      <div ref={panelRef} className="dialog" style={style} role="dialog" aria-modal="true" aria-labelledby={titleId} tabIndex={-1}>
        <div className="dialog-head" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
