...
export function ReceivePoDialog({ po, onClose }: { po: PurchaseOrder; onClose: () => void }) {
  ...
  const submit = () => {
    const res = receivePurchaseOrder(po.id, receipts)
    flash(res.ok ? t.poReceivedToast : t[refuseKey(res.reason)])
    if (res.ok) onClose()
  }
  ...
  return (
    <Dialog title={`${t.receivePo} — ${po.id}`} onClose={onClose} width={480}>
      ...
      {po.items.map((i) => {
        ...
        return (
          <Field key={i.productCode} label={`${p ? productName(lang, p) : i.productCode} — ${t.ordered} ${i.qtyOrdered} / ${t.received} ${i.qtyReceived}`}>
